...
from EcClairvaux import ECDSA as dsa
...
publicKey = dsa.generatePublicKey(privateKey, generator, curve)
message = "Hello, I am Bernard of Clairvaux"
signature = dsa.signDoc(message, privateKey, generator, curve)

assert dsa.validateDoc(message, publicKey, signature, generator, curve)
signature.s1+=1
assert not dsa.validateDoc(message, publicKey, signature, generator, curve)
