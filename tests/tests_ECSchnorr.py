...
from EcClairvaux import ECSchnorr as ecsh
...
privateKey = randint(0, curve.p-1)
publicKey = ecsh.generatePublicKey(privateKey, generator, curve)
message = "Hello, I am Bernard of Clairvaux"
signature = ecsh.signDoc(message, privateKey, generator, curve)

assert ecsh.validateDoc(message, publicKey, signature, generator, curve)
signature.s+=1
assert not ecsh.validateDoc(message, publicKey, signature, generator, curve)
